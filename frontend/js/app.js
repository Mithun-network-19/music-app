 ...
 * Ensures music plays continuously without interruption when navigating between pages.
 */
...
function createSongCard(song, songsList) {
  ...
  // Click card to play track without interrupting navigation
 ...
 * Intercepts page navigation to keep audio playing continuously in the background
 */
async function navigateToPage(url, pushState = true) {
  ...
  try {
    ...
    if (!newContent) {
      window.location.href = url; // Fallback to normal load if structure differs
      return;
    }

    // Swap main content area dynamically
    ...
    // Re-initialize theme & active page controllers
    initTheme();
...
function initSPARouter() {
  document.body.addEventListener('click', (e) => {
    const link = e.target.closest('a.nav-link-spa, a.btn-primary, a.btn-secondary, a.see-all-btn');
    if (!link) return;
